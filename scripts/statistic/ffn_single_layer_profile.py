...
TOP_K_SUMMARY = 5
SEED = 1234
OUTPUT_DIR = Path(__file__).resolve().with_name("ffn_single_layer_profile_output")
...
def log_path_for_layer(output_dir, layer_idx):
    return output_dir / f"ffn_single_layer_profile_layer_{layer_idx}.log"
...
def main():
    ...
    for target_layer in range(num_layers):
        intermediate_size = model.model.layers[target_layer].mlp.intermediate_size
        state = init_profile_state(intermediate_size)

        cleanup_output_dir(OUTPUT_DIR, target_layer)
        log_path = open_log_file(log_path_for_layer(OUTPUT_DIR, target_layer))
        log_message(f"[diag] writing logs to: {log_path}")
        log_message(f"[diag] layer sweep progress: {target_layer + 1}/{num_layers}")

        try:
            generated_text = run_profile_pass(model, tokenizer, runtime_device, target_layer, state)
            counts_path, adjacency_path, adjacency = save_outputs(OUTPUT_DIR, target_layer, state)
